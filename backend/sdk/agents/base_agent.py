...
from backend.sdk.text_parser_mixin import TextParserMixin
from backend.sdk.errors import AgentMaxRetryError, LLMMaxRetryError
# from backend.sdk.color_print_mixin import ColorPrintMixin


class BaseAgent(ABC, TextParserMixin):
    NAME = None
    ...
    def from_llm_config(cls, llm_config, llm_backend='openai', max_api_call_retries=3, max_agent_act_retries=1, *args, **kwargs):
        if llm_backend == 'openai':
            ...
            llm = GroqLLM(llm_config, max_api_call_retries)
        else:
